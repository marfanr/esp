...
void vTaskCode(void *pvParameters) {
  while (true) {
    ...
    ESP_LOGI("Main", "Soil Phosphorus: %" PRIu16, soil_params.soil_phosporus_value);
    ESP_LOGI("Main", "Soil Potassium: %" PRIu16, soil_params.soil_potassium_value);
...
void app_main(void) {
  ...
  xTaskCreatePinnedToCore(&vTaskCode, "read_sensor", 1024, NULL,
